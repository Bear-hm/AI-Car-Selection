...
function addCaution(cautions, text) {
  if (text && !cautions.includes(text)) {
    cautions.push(text)
  }
}
...
function scoreBudget(user, car, reasons, cautions) {
  ...
  if (budgetRelation.type === 'above') {
    if (budgetRelation.gap <= 1.5) {
      addReason(reasons, '价格接近预算上限')
      addCaution(cautions, '略超预算')
      return {
        score: 18,
        budgetRelation
      }
    ...
    if (budgetRelation.gap <= 3) {
      addReason(reasons, '预算略超，可作升级备选')
      addCaution(cautions, '超出预算不多')
      return {
        score: 10,
        budgetRelation
      }
...
function scoreUsage(user, car, reasons) {
  if (user.usage === 'mixed') {
    if (car.usageTags.length >= 3) {
      addReason(reasons, '多场景适配')
      return 18
    }

    if (car.usageTags.length >= 2) {
      addReason(reasons, '场景覆盖较全面')
      return 12
    }
...
function scorePeople(user, car, reasons) {
  if (user.people === '5_plus') {
    if (car.seats >= 7) {
      addReason(reasons, '多人乘坐更从容')
      return 18
    }

    if (car.seats >= 5) {
      addReason(reasons, '适合5人以上乘坐')
      return 15
    }
  ...
  if (user.people === '3_4') {
    if (car.seats >= 5 && car.spaceLevel >= 4) {
      addReason(reasons, '家用乘坐更宽松')
      return 12
    }

    if (car.seats >= 5) {
      addReason(reasons, '3-4人乘坐基本合适')
      return 8
    }
...
function scoreCharge(user, car, reasons, cautions) {
  if (!user.chargeRequired) {
    if (car.energy === 'electric') {
      addCaution(cautions, '无充电条件，纯电便利性一般')
      return -20
    }

    if (car.chargeRequired) {
      addCaution(cautions, '更依赖充电条件')
      return -8
    }

    addReason(reasons, '补能更省心')
    return 4
  }

  if (car.chargeRequired) {
    addReason(reasons, '有充电条件更适配')
    return 5
  }
...
function scorePriority(user, car, reasons) {
  ...
  if (user.priority === 'space') {
    if (car.spaceLevel >= 5) {
      score += 12
      addReason(reasons, '空间表现很强')
    } else if (car.spaceLevel >= 4) {
      score += 10
      addReason(reasons, '空间表现较好')
  ...
  if (user.priority === 'smart') {
    if (car.smartLevel >= 5) {
      score += 12
      addReason(reasons, '智能配置很强')
    } else if (car.smartLevel >= 4) {
      score += 10
      addReason(reasons, '智能配置较强')
  ...
  if (user.priority === 'value') {
    if (car.costLevel <= 2) {
      score += 12
      addReason(reasons, '用车成本更友好')
    } else if (car.costLevel === 3) {
      score += 6
      addReason(reasons, '成本表现比较均衡')
  ...
  if (user.priority === 'fuel_economy') {
    if (car.advantageTags.includes('fuel_economy')) {
      score += 10
      addReason(reasons, '能耗表现更好')
    }

    if (car.energy === 'hybrid') {
      score += 5
      addReason(reasons, '混动更适合省油诉求')
...
function scoreTypePreference(user, car, reasons) {
  ...
  if (car.type === user.carType) {
    addReason(reasons, '符合' + getTypeLabel(user.carType) + '偏好')
    return 8
  }

  if (user.carType === 'suv' && car.type === 'mpv') {
    addReason(reasons, '虽不是SUV，但实用取向接近')
    return 4
  }
...
function scoreEnergyPreference(user, car, reasons) {
  ...
  if (car.energy === user.energy) {
    addReason(reasons, '符合' + getEnergyLabel(user.energy) + '偏好')
    return 8
  }

  if (user.energy === 'hybrid' && car.energy === 'electric') {
    addReason(reasons, '同属新能源方向')
    return 4
  }

  if (user.energy === 'electric' && car.energy === 'hybrid') {
    addReason(reasons, '新能源取向接近')
    return 3
  }
...
export function scoreCars(user, cars) {
...
export function getCarEnergyLabel(energy) {
  return getEnergyLabel(energy)
}
